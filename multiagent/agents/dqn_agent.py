...
class DQNAgent(object):
    ...
    def step_env(self):
        ...
        # TADA store the latest observation into the replay buffer
        # HINT: see replay buffer's function store_frame
        self.replay_buffer_idx = self.replay_buffer.store_frame(self.last_obs)

        eps = self.exploration.value(self.t)
        # TADA use epsilon greedy exploration when selecting action
        ...
        if perform_random_action:
            ...
            # TADA query the policy to select action
            ...
            # TADA query the policy with enc_last_obs to select action
            action = self.actor.get_action(enc_last_obs)
            action = action[0]

        # TADA take a step in the environment using the action from the policy
        ...
        # TADA store the result of taking this action into the replay buffer
        # HINT1: see replay buffer's store_effect function
        # HINT2: one of the arguments you'll need to pass in is self.replay_buffer_idx from above
        self.replay_buffer.store_effect(self.replay_buffer_idx, action, reward, done)

        # TADA if taking this step resulted in done, reset the env (and the latest observation)
        if done:
            self.last_obs = self.env.reset()
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        if (self.t > self.learning_starts and \
            ...
            # TADA populate all placeholders necessary for calculating the critic's total_error
            ...
            # TADA: create a LIST of tensors to run in order to 
            ...
            # TADA: use sess.run to periodically update the critic's target function
