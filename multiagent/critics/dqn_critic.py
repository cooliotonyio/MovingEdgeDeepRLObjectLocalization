...
from multiagent.util.dqn_utils import minimize_and_clip, huber_loss
...
class DQNCritic(BaseCritic):
    ...
    def _build(self, q_func):
        ...
        if self.double_q:
            ...
            # q_tp1 = q_tp1_values[tf.argmax(self.q_t_values)]
            action_inds = tf.argmax(self.q_t_values, axis=1)
            flat_q_tp1_vals = tf.reshape(q_tp1_values, [-1])
            flat_ind_max = action_inds + tf.cast(tf.range(tf.shape(q_tp1_values)[0]) * tf.shape(q_tp1_values)[1], tf.int64)
            q_tp1 = tf.gather(flat_q_tp1_vals, flat_ind_max)
        ...
        # TADA calculate the targets for the Bellman error
        ...
        # TADA compute the Bellman error (i.e. TD error between q_t and target_q_t)
        ...
        # TADA these variables should all of the 
